...
enum OTPError: Error {
    case unableToGenerateKey(_ size: OTPKeySize)
    case invalidKey
    case invalidToken
}

public enum OTPKeySize: Int {
    case key20 = 20
    case key40 = 40
}
...
public extension String {
    ...
    static func generateOTPKey(size: OTPKeySize) throws -> String {
        guard let key = randomString(length: size.rawValue).base32String
        else {
            throw OTPError.unableToGenerateKey(size)
        }
        return key
    }
...
public extension String {
    ...
    func getOTPToken(interval: Int) throws -> String {
        let totp = try getOTOP(interval: interval)
        return totp.generate(time: Date())
    }
    
    func getOTPTokens(interval: Int, range: Int) throws -> [String] {
        let totp = try getOTOP(interval: interval)
        return totp.generate(time: Date(), range: range)
    }
}

private extension String {
    
    func getOTOP(interval: Int) throws -> TOTP {
        guard let secretData = Data(base64Encoded: self)
        else {
            throw OTPError.invalidKey
        }
        
        let key = SymmetricKey(data: secretData)
        let totp = TOTP(key: key, digest: .sha256, digits: .six, interval: interval)
        return totp
    }
}
...
private extension String {
    ...
    var base32String: String? {
        ...
        let base32EncodedData = data.base64EncodedData()
